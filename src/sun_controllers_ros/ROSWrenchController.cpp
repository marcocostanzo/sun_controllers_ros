...
*/
sun::ROSWrenchController::ROSWrenchController(ros::NodeHandle &nh_public,
  ...
  // Subscribers
  sub_wrench_command_ =
      nh_public.subscribe(wrench_command_topic_str_, 1,
                          &ROSWrenchController::wrench_command_cb_, this);
  sub_wrench_measure_ =
      nh_public.subscribe(wrench_measure_topic_str_, 1,
                          &ROSWrenchController::wrench_measure_cb_, this);

  // Publishers
  pub_twist_control_ = nh_public.advertise<geometry_msgs::TwistStamped>(
      twist_command_topic_str_, 1);

  // Service Servers
  srv_server_set_enable_ = nh_public.advertiseService(
      service_set_enable_str_, &ROSWrenchController::srv_server_set_enabled_cb_,
      this);
}
...
sun::ROSWrenchController::compute_measure_mean(int num_samples) {
...
geometry_msgs::TwistStamped sun::ROSWrenchController::compute_control() {
  ...
  twist.twist.linear.x = gains_[0] * (wrench_command_.wrench.force.x -
                                      wrench_measure_.wrench.force.x);
  twist.twist.linear.y = gains_[1] * (wrench_command_.wrench.force.y -
                                      wrench_measure_.wrench.force.y);
  twist.twist.linear.z = gains_[2] * (wrench_command_.wrench.force.z -
                                      wrench_measure_.wrench.force.z);
  twist.twist.angular.x = gains_[3] * (wrench_command_.wrench.torque.x -
                                       wrench_measure_.wrench.torque.x);
  twist.twist.angular.y = gains_[4] * (wrench_command_.wrench.torque.y -
                                       wrench_measure_.wrench.torque.y);
  twist.twist.angular.z = gains_[5] * (wrench_command_.wrench.torque.z -
                                       wrench_measure_.wrench.torque.z);
  return twist;
}
...
*/
void sun::ROSWrenchController::get_ros_params_gains(
    ros::NodeHandle &nh_private) {
  if (nh_private.hasParam("gains")) {
    std::vector<double> gains_std;
    if (!nh_private.getParam("gains",
                             gains_std)) { // Param is not std::vector<double>
      ROS_ERROR("[Force Controller] INVALID PARAM TYPE: gains has to be a "
                "vector [...]");
      ros::shutdown();
      exit(-1);
    }
...
*/
void sun::ROSWrenchController::get_ros_params_topics_services_name(
    ros::NodeHandle &nh_private) {
  nh_private.param("wrench_command_topic", wrench_command_topic_str_,
                   std::string("wrench_command"));
  nh_private.param("wrench_measure_topic", wrench_measure_topic_str_,
                   std::string("wrench_measure"));
  nh_private.param("twist_command_topic", twist_command_topic_str_,
                   std::string("twist_command"));
  nh_private.param("service_set_enable", service_set_enable_str_,
                   std::string("set_enable"));
}
...
bool sun::ROSWrenchController::srv_server_set_enabled_cb_(
  ...
  if (request.data != enabled_) { // If status changed
    if (request.data) {           // if enabled_ requested
      pre_start_();
...
/*
    ...
    Update the command
*/
void sun::ROSWrenchController::wrench_command_cb_(
...
*/
void sun::ROSWrenchController::wrench_measure_cb_(
    const geometry_msgs::WrenchStamped::ConstPtr &msg) {
  // Update measure
  wrench_measure_ = *msg;
...
/*
    ...
    and publish it
*/
void sun::ROSWrenchController::publish_control_(
